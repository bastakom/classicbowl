  ...
  return (
    <div className="mt-10 lg:mt-0 flex flex-col justify-center items-center mb-20">
      <div className="flex gap-16 uppercase text-[16px] italic font-extrabold">
        <div
          className={`cursor-pointer transition-colors ${item === 1 ? "text-[#666666]" : "text-black"
            }`}
          onClick={() => setItem(1)}
        >
          {blok.first_week}
        ...
        <div
          className={`cursor-pointer transition-colors ${item === 2 ? "text-[#666666]" : "text-black"
            }`}
          onClick={() => setItem(2)}
        >
          {blok.second_week}
        ...
        {item === 1
          ? blok.first_field.map((item: any, index: number) => (
            <div key={index}>
              ...
              {item.VEG &&
                <div className="mt-4">
                  <h3 className="text-[22px] font-semibold">{item.VEG}</h3>
                  <div>{item.VEG_description}</div>
                </div>
              }
            </div>
          ))
